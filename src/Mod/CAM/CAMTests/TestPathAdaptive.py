...
class TestPathAdaptive(PathTestBase):
    ...
    def test09(self):
        """test09() Tests full roughing- should machine entire model with no inputs"""
        # Instantiate a Adaptive operation and set Base Geometry
        adaptive = PathAdaptive.Create("Adaptive")
        adaptive.Base = [(self.doc.Fusion, [])]  # (base, subs_list)
        adaptive.Label = "test09+"
        adaptive.Comment = "test09() Verify path generated with no subs roughs entire model"
    ...
    def test10(self):
        """test10() Tests stock handling- should rough full model, but not cut
        air excessively where there's not stock"""
        # Instantiate a Adaptive operation and set Base Geometry
        adaptive = PathAdaptive.Create("Adaptive")
        adaptive.Base = [(self.doc.Fusion, [])]  # (base, subs_list)
        adaptive.Label = "test10+"
        adaptive.Comment = "test10() Verify machining region is limited to the stock"
    ...
    def test11(self):
        """test11() Tests Z stock to leave- with 1mm Z stock to leave, machining
        at the top of the model should not touch the top model face"""
        # Instantiate a Adaptive operation and set Base Geometry
        adaptive = PathAdaptive.Create("Adaptive")
        adaptive.Base = [(self.doc.Fusion, [])]  # (base, subs_list)
        adaptive.Label = "test11+"
        adaptive.Comment = "test11() Verify Z stock is left as requested"
